...
def authenticate_user(username: str, password: str):
    sql = "EXEC dbo.spLoginConfirmation_Get @Username=:username"
    # sql = "EXEC dbo.spUserForAuth_Get @UserName=:username"
    user = data_access.load_query_single_with_params(sql, {"username": username})
...
async def login(user_for_login: UserLogin):
    ...
    user = authenticate_user(user_for_login.username, decrypted_pw)
    if not user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid username or password")
    
    # Create JWT token
    access_token = create_access_token(data={"username": user_for_login.username})
